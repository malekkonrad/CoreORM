...
public class NewSingleTableInheritanceStrategy extends AbstractInheritanceStrategy {

    public NewSingleTableInheritanceStrategy(EntityMetadata metadata) {super(metadata);}
    ...
    public Pair<String, String> create() {
        ...
        if (entityMetadata.getInheritanceMetadata().getChildren().isEmpty()) {
    ...
    private EntityMetadata findConcreteMetadata(Object id, Session session) throws Exception {
        ...
        List<EntityMetadata> concreteSubclasses = getAllConcreteSubclassesLEAFES(entityMetadata);
    ...
    public <T> List<T> findAll(Class<T> type, Session session, PairTargetStatements pairTargetStatements) {
        ...
        List<EntityMetadata> concreteSubclasses = getAllConcreteSubclassesLEAFES(this.entityMetadata);
    ...
    public <T> List<T> findBy(Class<T> type, Session session, QuerySpec<T> querySpec) {
        ...
        List<EntityMetadata> allSubclasses = getAllConcreteSubclassesLEAFES(entityMetadata);
    ...
    private List<EntityMetadata> getAllConcreteSubclassesLEAFES(EntityMetadata parent) {
        List<EntityMetadata> result = new ArrayList<>();
        List<EntityMetadata> toVisit = new ArrayList<>();

        toVisit.add(parent);

//        if (parent.getInheritanceMetadata().getChildren().isEmpty()) {
//            result.add(parent);
//        }
        while (!toVisit.isEmpty()) {
            var current =  toVisit.remove(0);
            if (current.getInheritanceMetadata().getChildren().isEmpty()) {
                result.add(current);
            }
            for (EntityMetadata child: current.getInheritanceMetadata().getChildren()){
                toVisit.add(child);
