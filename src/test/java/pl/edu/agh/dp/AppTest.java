...
public class AppTest {
        ...
        //// try (Connection conn =
        // DriverManager.getConnection("jdbc:sqlite:src/test/resources/test.db");
        //// Statement stmt = conn.createStatement();
        //// ResultSet rs = stmt.executeQuery("SELECT * FROM users WHERE id = 1")) {
        //// if (rs.next()) {
        //// System.out.println("FOUND IN DB: id=" + rs.getLong("id") + ", name=" +
        // rs.getString("name"));
        //// } else {
        ...
        public void testAutoIncrement() {
                ...
                // try (Connection conn = DriverManager.getConnection(url, user, password);
                // Statement stmt = conn.createStatement();
                // ResultSet rs = stmt.executeQuery("SELECT * FROM huskys WHERE id = " +
                // dog.getId())) {
                // if (rs.next()) {
                // System.out.println("FOUND IN DB: id=" + rs.getLong("id") + ", name=" +
                // rs.getString("name") + ", how=" + rs.getString("how"));
                // } else {
                // System.out.println("NOT FOUND IN DB!");
                // }
                // } catch (SQLException e) {
